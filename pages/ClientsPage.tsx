...
const ClientsPage: React.FC<ClientsPageProps> = ({ showContextMenu, onOpenAdminTaskModal, onCreateInvoice }) => {
    ...
    React.useEffect(() => { setIsContactPickerSupported('contacts' in navigator && 'ContactsManager' in window); }, []);
    ...
    const filteredClients = React.useMemo(() => {
        ...
        return (clients || []).map(client => {
            ...
            if (client.name.toLowerCase().includes(lowercasedQuery) || client.contactInfo.toLowerCase().includes(lowercasedQuery)) return client;
            if (matchingCases.length > 0) return { ...client, cases: matchingCases };
            return null;
    ...
    const handleOpenModal = (type: 'client' | 'case' | 'stage' | 'session', isEditing = false, context: any = {}) => {
        ...
        if (isEditing && context.item) {
            ...
            if (type === 'session') setFormData({ ...item, date: toInputDateString(item.date), nextSessionDate: toInputDateString(item.nextSessionDate) });
            else if (type === 'stage') {
                const { firstSessionDate, decisionDate, ...restOfStage } = item;
                setFormData({ ...restOfStage, firstSessionDate: toInputDateString(firstSessionDate), decisionDate: toInputDateString(decisionDate) });
            } else setFormData(item);
        } else setFormData(context.id ? { [`${type}Id`]: context.id } : {});
    };

    const handleCloseModal = () => { setModal({ type: null, isEditing: false }); setFormData({}); };
    ...
    const handleImportContact = async () => {
        try {
            ...
            if (contacts.length > 0) {
                ...
                setFormData((prev: any) => ({ ...prev, name: contact.name?.[0] || prev.name, contactInfo: contact.tel?.[0] || prev.contactInfo }));
            }
        } catch (ex) { console.error("Contact import failed", ex); }
    };
        ...
        setClients(currentClients => (currentClients || []).map(client => ({
            ...
            cases: (client.cases || []).map(caseItem => ({
                ...
                stages: (caseItem.stages || []).map(stage => {
                    ...
                    updatedSessions[sessionIndex] = { ...updatedSessions[sessionIndex], ...updatedFields, updated_at: new Date() };
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (type === 'client') {
            ...
            if (!clientName) { alert("اسم الموكل مطلوب."); return; }
            const normalizedClientName = clientName.toLowerCase();
            const foundClient = (clients || []).find(c => c.name.trim().toLowerCase() === normalizedClientName);
            if (foundClient && (!isEditing || (isEditing && context?.item?.id !== foundClient.id))) { alert(`تنبيه: الموكل "${clientName}" موجود بالفعل.`); return; }
            if (isEditing) {
                 if (context?.item?.id) setClients(prev => (prev || []).map(c => c.id === context.item.id ? { ...c, ...formData, name: clientName, updated_at: new Date() } : c));
            } else {
                const newClient: Client = { id: `client-${Date.now()}`, name: clientName, contactInfo: formData.contactInfo || '', cases: [], updated_at: new Date() };
                setClients(prev => [...(prev || []), newClient]);
            }
        } else if (type === 'case') {
            if (isEditing) {
                setClients(prev => (prev || []).map(c => c.id === context.client.id ? { ...c, updated_at: new Date(), cases: (c.cases || []).map(cs => cs.id === context.item.id ? { ...cs, ...formData, updated_at: new Date() } : cs) } : c));
            } else {
                const clientForCase = (clients || []).find(c => c.id === context.clientId);
                if (clientForCase) {
                    const newCase: Case = { id: `case-${Date.now()}`, subject: formData.subject || 'قضية بدون موضوع', opponentName: formData.opponentName || '', feeAgreement: formData.feeAgreement || '', status: formData.status || 'active', clientName: clientForCase.name, stages: [], updated_at: new Date() };
                    ...
                    if (court || caseNumber || firstSessionDate) {
                        ...
                        const newStage: Stage = { id: `stage-${Date.now()}`, court: court || 'غير محدد', caseNumber: caseNumber || '', firstSessionDate: parsedFirstSessionDate || undefined, sessions: [], updated_at: new Date() };
                        if (parsedFirstSessionDate) newStage.sessions.push({ id: `session-${Date.now()}-first`, court: newStage.court, caseNumber: newStage.caseNumber, date: parsedFirstSessionDate, clientName: clientForCase.name, opponentName: newCase.opponentName, isPostponed: false, postponementReason: firstSessionReason || undefined, assignee: 'بدون تخصيص', updated_at: new Date() });
            ...
            if (isEditing) {
                ...
                setClients(prev => (prev || []).map(c => c.id === context.client.id ? { ...c, updated_at: new Date(), cases: (c.cases || []).map(cs => cs.id === context.case.id ? { ...cs, updated_at: new Date(), stages: (cs.stages || []).map(st => st.id === context.item.id ? { ...st, ...stageData, updated_at: new Date() } : st) } : cs) } : c));
            } else {
                const stageData = { ...formData };
                const parsedFirstSessionDate = parseInputDateString(stageData.firstSessionDate);
                const newStage: Stage = { id: `stage-${Date.now()}`, court: stageData.court || 'غير محدد', caseNumber: stageData.caseNumber || '', firstSessionDate: parsedFirstSessionDate || undefined, sessions: [], updated_at: new Date() };
                if (newStage.firstSessionDate) {
                    ...
                    if (client && caseItem) newStage.sessions.push({ id: `session-${Date.now()}-first`, court: newStage.court, caseNumber: newStage.caseNumber, date: newStage.firstSessionDate, clientName: client.name, opponentName: caseItem.opponentName, isPostponed: false, postponementReason: stageData.firstSessionReason || undefined, assignee: 'بدون تخصيص', updated_at: new Date() });
                }
                setClients(prev => (prev || []).map(c => c.id === context.clientId ? { ...c, updated_at: new Date(), cases: (c.cases || []).map(cs => cs.id === context.caseId ? { ...cs, updated_at: new Date(), stages: [...(cs.stages || []), newStage] } : cs) } : c));
            }
        } else if (type === 'session') {
            if (isEditing) {
                 const sessionData = { ...formData };
                 const parsedDate = parseInputDateString(sessionData.date);
                 if (!parsedDate) { alert("تاريخ الجلسة غير صالح."); return; }
                 sessionData.date = parsedDate;
                 sessionData.nextSessionDate = parseInputDateString(sessionData.nextSessionDate) || undefined;
                 setClients(prev => (prev || []).map(c => c.id === context.client.id ? { ...c, updated_at: new Date(), cases: (c.cases || []).map(cs => cs.id === context.case.id ? { ...cs, updated_at: new Date(), stages: (cs.stages || []).map(st => st.id === context.stage.id ? { ...st, updated_at: new Date(), sessions: (st.sessions || []).map(s => s.id === context.item.id ? { ...s, ...sessionData, updated_at: new Date() } : s) } : st) } : cs) } : c));
            } else {
                ...
                if (client && caseItem && stage) {
                    const newSession: Session = { id: `session-${Date.now()}`, date: parsedDate, court: stage.court, caseNumber: stage.caseNumber, clientName: client.name, opponentName: caseItem.opponentName, isPostponed: false, assignee: formData.assignee || 'بدون تخصيص', updated_at: new Date() };
                    setClients(prev => (prev || []).map(c => c.id === context.clientId ? { ...c, updated_at: new Date(), cases: (c.cases || []).map(cs => cs.id === context.caseId ? { ...cs, updated_at: new Date(), stages: (cs.stages || []).map(st => st.id === context.stage.id ? { ...st, sessions: [...(st.sessions || []), newSession], updated_at: new Date() } : st) } : cs) } : c));
                }
            }
        }
        handleCloseModal();
    ...
    const handleDeleteClient = (client: Client) => { setClientToDelete(client); setIsDeleteClientModalOpen(true); };
    const handleConfirmDeleteClient = () => { if (clientToDelete) deleteClient(clientToDelete.id); setIsDeleteClientModalOpen(false); setClientToDelete(null); };
    const handleDeleteCase = (caseId: string, clientId: string, caseSubject: string) => { setCaseToDelete({ caseId, clientId, caseSubject }); setIsDeleteCaseModalOpen(true); };
    const handleConfirmDeleteCase = () => { if (caseToDelete) deleteCase(caseToDelete.caseId, caseToDelete.clientId); setIsDeleteCaseModalOpen(false); setCaseToDelete(null); };
    const handleDeleteStage = (stageId: string, caseId: string, clientId: string) => {
        ...
        setStageToDelete({ stageId, caseId, clientId, stageInfo: stage ? `${stage.court} (${stage.caseNumber})` : 'هذه المرحلة' });
        setIsDeleteStageModalOpen(true);
    };
    const handleConfirmDeleteStage = () => { if (stageToDelete) deleteStage(stageToDelete.stageId, stageToDelete.caseId, stageToDelete.clientId); setIsDeleteStageModalOpen(false); setStageToDelete(null); };
    const handleDeleteSession = (sessionId: string, stageId: string, caseId: string, clientId: string) => {
        ...
        setSessionToDelete({ sessionId, stageId, caseId, clientId, message: session ? `جلسة يوم ${formatDate(session.date)} الخاصة بقضية ${caseItem?.subject}` : 'هذه الجلسة' });
        setIsDeleteSessionModalOpen(true);
    };
    const handleConfirmDeleteSession = () => { if (sessionToDelete) deleteSession(sessionToDelete.sessionId, sessionToDelete.stageId, sessionToDelete.caseId, sessionToDelete.clientId); setIsDeleteSessionModalOpen(false); setSessionToDelete(null); };
    const handlePrintClientStatement = (clientId: string) => { const client = (clients || []).find(c => c.id === clientId); if (client) { setClientForPrintChoice(client); setIsPrintChoiceModalOpen(true); } };
    const handleGeneratePrintData = (client: Client, caseData?: Case) => {
        const entries = caseData ? (accountingEntries || []).filter(e => e.caseId === caseData.id) : (accountingEntries || []).filter(e => e.clientId === client.id);
        const income = entries.filter(e => e.type === 'income').reduce((sum, e) => sum + e.amount, 0);
        const expense = entries.filter(e => e.type === 'expense').reduce((sum, e) => sum + e.amount, 0);
        setPrintData({ client, caseData, entries, totals: { income, expense, balance: income - expense } });
        setIsPrintChoiceModalOpen(false); setIsPrintModalOpen(true);
    };
    const handleOpenDecideModal = (session: Session) => {
        if (!session.stageId) return;
        let foundStage: Stage | null = null;
        for (const client of (clients || [])) { for (const caseItem of (client.cases || [])) { const stage = (caseItem.stages || []).find(st => st.id === session.stageId); if (stage) { foundStage = stage; break; } } if (foundStage) break; }
    ...
    const handleDecideSubmit = (e: React.FormEvent) => {
        ...
        setClients(currentClients => (currentClients || []).map(client => ({
            ...client, updated_at: new Date(),
            cases: (client.cases || []).map(c => ({ ...c, updated_at: new Date(), stages: (c.stages || []).map(st => st.id === stage.id ? { ...st, decisionDate: session.date, decisionNumber: decideFormData.decisionNumber, decisionSummary: decideFormData.decisionSummary, decisionNotes: decideFormData.decisionNotes, updated_at: new Date() } : st) }))
        })));
        handleCloseDecideModal();
    };
    const getModalTitle = () => { const { type, isEditing } = modal; if (!type) return ''; const action = isEditing ? 'تعديل' : 'إضافة'; switch (type) { case 'client': return `${action} موكل`; case 'case': return `${action} قضية`; case 'stage': return `${action} مرحلة تقاضي`; case 'session': return `${action} جلسة`; default: return ''; } };
    ...
    return (
        <div className="space-y-6">
            ...
            <div>
                {viewMode === 'tree' ? (
                    <ClientsTreeView 
                        clients={filteredClients} setClients={setClients} accountingEntries={accountingEntries} setAccountingEntries={setAccountingEntries} assistants={assistants} 
                        ...
                        onPrintClientStatement={handlePrintClientStatement} onPostponeSession={permissions.can_postpone_session ? postponeSession : undefined} onUpdateSession={permissions.can_edit_session ? onUpdateSession : undefined} onDecide={permissions.can_decide_session ? handleOpenDecideModal : undefined} showContextMenu={showContextMenu} onOpenAdminTaskModal={onOpenAdminTaskModal} onCreateInvoice={onCreateInvoice} permissions={permissions} 
                    />
                ) : (
                    <ClientsListView 
                        clients={filteredClients} setClients={setClients} accountingEntries={accountingEntries} setAccountingEntries={setAccountingEntries} assistants={assistants}
                    ...
                    <div className="bg-white p-6 rounded-lg shadow-xl w-full max-w-lg" onClick={e => e.stopPropagation()}>
                        ...
                        <form onSubmit={handleSubmit} className="space-y-4">
                            {modal.type === 'client' && (
                                <>
                                <div>
                                    <div className="flex justify-between items-center"><label className="block text-sm font-medium">اسم الموكل</label>
                                        {isContactPickerSupported && <button type="button" onClick={handleImportContact} className="text-xs flex items-center gap-1 text-blue-600 hover:text-blue-800 transition-colors"><AddressBookIcon className="w-4 h-4" />استيراد من جهات الاتصال</button>}
                                    </div>
